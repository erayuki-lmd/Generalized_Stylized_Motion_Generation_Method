...
class ForwardKinematics:
    ...
    def forward_from_raw(self, rotation, world=True, quater=True):
        if quater:
            ...
            ones = torch.ones(norm.shape, device=rotation.device, dtype=norm.dtype) * 1e-10
    ...
    def forward(self, rotation: torch.Tensor, position: torch.Tensor, order='xyz', quater=True, world=True):
        ...
        if quater and rotation.shape[-2] != 3: raise Exception('Unexpected shape of rotation')
        ...
        for i, pi in enumerate(self.topology):
            if pi == -1:
                assert i == 0
                continue
            len_bone = torch.sqrt((self.offset[i][0]*self.offset[i][0])+(self.offset[i][1]*self.offset[i][1])+(self.offset[i][2]*self.offset[i][2]))
            #print(len_bone)
            result[..., i, :] = rotation[..., :, i, :]*len_bone
            result[..., i, :] += result[..., pi, :]
        # print("########################")
        # for j in rotation[0][0]:
        #     print(str(float(j[0]))+";"+str(float(j[1]))+";"+str(float(j[2])))
        # print("########################")
        # for j in result[0][0]:
        #     print(str(float(j[0]))+";"+str(float(j[1]))+";"+str(float(j[2])))
        return result
    ...
    # def forwardX(self, rotations):
    #     local_pos = self.forward_from_raw(rotations, world=True)  # [B, T, J, 3]
    #     local_pos = self.trim(local_pos)
    #     return local_pos
