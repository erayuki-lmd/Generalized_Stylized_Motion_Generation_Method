...
#CHANGE!!!!#######################################
def forward_rotations_2(skel, rotations, rtpos=None, trim=True):
    """
    input: rotations [T, J, 4], rtpos [T, 3]
    output: positions [T, J, 3]
    """
...
class AnimationData:
    ...
    #CHANGE!!!!#######################################
    def get_joint_direction(self):
        pos3d = forward_rotations_2(self.skel, self.rotations, trim=False)  # [T, J - 1, 3]
        for i in range(len(pos3d)):
            for j in range(len(pos3d[i])):
                norm = np.sqrt((pos3d[i][j][0])**2 + (pos3d[i][j][1])**2 + (pos3d[i][j][2])**2)
                if norm != 0:
                    pos3d[i][j][0] /= norm
                    pos3d[i][j][1] /= norm
                    pos3d[i][j][2] /= norm
